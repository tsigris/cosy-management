...
function DashboardContent() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setTransactions([]); // Reset για να μη βλέπουμε παλιά δεδομένα
      ...
      if (storeErr || !storeData) {
        throw new Error("Το κατάστημα δεν βρέθηκε");
      }
      
      setStoreName(storeData.name);

      // ΦΟΡΤΩΣΗ ΚΙΝΗΣΕΩΝ - ΜΕ ΑΣΦΑΛΕΣ JOIN (!left)
      const { data: tx, error: txError } = await supabase
        .from('transactions')
        .select(`
          *,
          suppliers!left(name),
          fixed_assets!left(name)
        `)
      ...
      toast.error(`Σφάλμα: ${err.message || 'Αποτυχία φόρτωσης'}`);
    } finally {
      setLoading(false);
    }
  ...
  const totals = useMemo(() => {
    ...
    const expense = transactions.filter(t => (t.type === 'expense' || t.type === 'debt_payment') && !t.is_credit).reduce((acc, t) => acc + (Math.abs(Number(t.amount)) || 0), 0)
      ...
      <header style={headerStyle}>
        <div style={brandArea}>
          ...
          <div>
            ...
            <div style={dashboardSub}>BUSINESS DASHBOARD</div>
          </div>
        </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          {isMenuOpen && (
            <div style={dropdownStyle}>
              ...
              <button onClick={() => supabase.auth.signOut().then(() => router.push('/login'))} style={logoutBtnStyle}>ΑΠΟΣΥΝΔΕΣΗ 🚪</button>
            </div>
          )}
        </div>
      </header>
...
const switchBtnStyle: any = { fontSize: '9px', fontWeight: '800', color: colors.accentBlue, backgroundColor: '#eef2ff', padding: '4px 8px', borderRadius: '8px', textDecoration: 'none' };
const dashboardSub = { fontSize: '9px', fontWeight: '800', color: colors.secondaryText };
const menuToggle: any = { background: 'white', border: `1px solid ${colors.border}`, borderRadius: '12px', width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' };
...
const menuItem: any = { display: 'block', padding: '12px 15px', textDecoration: 'none', color: colors.primaryDark, fontWeight: '700', fontSize: '14px' };
...
const logoutBtnStyle: any = { width: '100%', textAlign: 'left', padding: '12px 15px', background: '#fff1f2', color: colors.accentRed, border: 'none', borderRadius: '12px', fontWeight: '700' };
...
const dateNavBtn = { background: 'none', border: 'none', color: colors.secondaryText };
const heroCardStyle: any = { background: colors.primaryDark, padding: '30px 20px', borderRadius: '28px', color: 'white', marginBottom: '30px', textAlign: 'center' };
const heroLabel: any = { fontSize: '10px', fontWeight: '700', opacity: 0.5, marginBottom: '10px' };
...
const actionBtn: any = { flex: 1, padding: '18px', borderRadius: '18px', color: 'white', textDecoration: 'none', textAlign: 'center', fontWeight: '800', fontSize: '14px' };
const listContainer = { backgroundColor: 'transparent' };
const listHeader = { fontSize: '11px', fontWeight: '900', color: colors.secondaryText, marginBottom: '15px' };
const txRow: any = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', backgroundColor: 'white', border: `1px solid ${colors.border}` };
...
const txMeta = { fontSize: '11px', color: colors.secondaryText, margin: 0 };
...
const emptyStateStyle: any = { textAlign: 'center', padding: '40px 20px', color: colors.secondaryText, fontWeight: '600' };
