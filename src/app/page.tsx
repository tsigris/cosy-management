...
function DashboardContent() {
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto', fontFamily: 'sans-serif' }}>
      ...
      {/* HEADER ΜΕ ΜΟΝΤΕΡΝΟ ΚΟΥΜΠΙ ΜΕΝΟΥ */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', paddingTop: '10px' }}>
        <h1 style={{ fontWeight: '900', fontSize: '26px', margin: 0, letterSpacing: '-1px', color: '#0f172a' }}>ΚΑΤΑΣΤΗΜΑ</h1>
        ...
        <div style={{ position: 'relative' }}>
          ...
          {isMenuOpen && (
            <div style={dropdownStyle}>
              ...
              <Link href="/cards" style={menuItem} onClick={() => setIsMenuOpen(false)}>🚩 Καρτέλες (Χρέη)</Link>
              ...
              <button onClick={() => alert('Logout...')} style={logoutBtnStyle}>ΑΠΟΣΥΝΔΕΣΗ 🚪</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <p style={{ fontSize: '11px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase' }}>Κινήσεις</p>
          ...
          <div key={t.id} style={itemStyle}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <p style={{ 
                fontWeight: '900', 
                fontSize: '16px', 
                color: t.is_credit ? '#94a3b8' : (t.type === 'income' ? '#16a34a' : '#dc2626'), 
                margin: 0 
              }}>
...
const btnStyle = { flex: 1, padding: '18px', borderRadius: '16px', color: 'white', textDecoration: 'none', textAlign: 'center' as const, fontWeight: '800', fontSize: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' };
...
const logoutBtnStyle = { ...menuItem, color: '#ef4444', border: 'none', background: '#fee2e2', width: '100%', cursor: 'pointer', marginTop: '5px' };
