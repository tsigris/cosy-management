...
function DashboardContent() {
  ...
  const [isZExpanded, setIsZExpanded] = useState(false) // State για την ομαδοποίηση του Ζ
  ...
  // ΔΙΑΧΩΡΙΣΜΟΣ Ζ ΚΑΙ ΛΟΙΠΩΝ ΚΙΝΗΣΕΩΝ
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '0 auto', fontFamily: 'sans-serif', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <p style={{ fontSize: '11px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase' }}>Κινήσεις Καταστήματος</p>

        {loading ? <div style={{ textAlign: 'center', padding: '20px' }}>Φόρτωση...</div> : (
          <>
            {/* ΟΜΑΔΟΠΟΙΗΜΕΝΟ Ζ (Εμφανίζεται μόνο αν υπάρχει έστω και ένα Ζ) */}
            {zTotal > 0 && (
              <div style={{ marginBottom: '5px' }}>
                ...
                {/* ΑΝΑΛΥΣΗ Ζ ΜΕ ΠΟΣΟΣΤΑ (Ανοίγει με κλικ) */}
                {isZExpanded && (
                  <div style={zBreakdownPanel}>
                    ...
                    <button 
                       onClick={() => router.push(`/daily-z?date=${selectedDate}`)}
                       style={zEditFullBtn}
                    >
                      ΕΠΕΞΕΡΓΑΣΙΑ Ζ ✎
                    </button>
...
const zEditFullBtn = { width: '100%', padding: '10px', marginTop: '10px', backgroundColor: '#fef3c7', color: '#92400e', border: 'none', borderRadius: '10px', fontWeight: '800', fontSize: '11px', cursor: 'pointer' };
