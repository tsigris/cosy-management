...
function CreditsContent() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      setLoadingTx(true)
      const transRes = await supabase.from('transactions').select('id, store_id, created_at, date, type, amount, category, method, notes, description, is_credit, supplier_id, fixed_asset_id, revenue_source_id').eq('store_id', storeIdFromUrl)
      if (transRes.error) {
