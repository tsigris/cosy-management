...
function CreditsContent() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const schedRes = await supabase.from('scheduled_payments').select('*').eq('store_id', storeId)
    ...
    <div style={iphoneWrapper}>
      ...
      <div style={{ maxWidth: '520px', margin: '0 auto', paddingBottom: '120px' }}>
        <EconomicsHeaderNav
          ...
          rightControl={<Link href={`/?store=${storeId || ''}`} style={backBtnStyle}><ChevronLeft size={20} /></Link>}
        />
