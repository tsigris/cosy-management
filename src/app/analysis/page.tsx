...
import { 
  ...
  subMonths, subYears, isWithinInterval, startOfDay, endOfDay 
} from 'date-fns'
...
export default function AnalysisPage() {
  ...
  const filterByTime = (data: any[], type: string, refDate: Date) => {
    return data.filter(t => {
      ...
      if (type === 'today') return isWithinInterval(d, { start: startOfDay(refDate), end: endOfDay(refDate) })
    ...
    <main style={{ backgroundColor: '#f8fafc', minHeight: '100vh', padding: '16px', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: '500px', margin: '0 auto' }}>
        ...
        <div style={{...mainCard, backgroundColor: view === 'income' ? '#064e3b' : '#450a0a'}}>
          <p style={labelSmall}>ΣΥΝΟΛΟ {view === 'income' ? 'ΕΣΟΔΩΝ' : 'ΕΞΟΔΩΝ'}</p>
        ...
        {view === 'expenses' && topSuppliers.length > 0 && (
          <div style={sectionCard}>
            <h3 style={sectionTitle}>TOP ΠΡΟΜΗΘΕΥΤΕΣ (ΑΝΑ ΕΞΟΔΟ)</h3>
        ...
        {/* ΙΣΤΟΡΙΚΟ */}
        <div style={sectionCard}>
          <h3 style={sectionTitle}>ΙΣΤΟΡΙΚΟ ΠΕΡΙΟΔΟΥ</h3>
