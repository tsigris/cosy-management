  ...
  const fetchBalances = useCallback(async () => {
    ...
    try {
      ...
      const [supsRes, transRes] = await Promise.all([
        supabase.from('suppliers').select('*').eq('store_id', storeIdFromUrl).order('name'),
        supabase.from('transactions').select('*').eq('store_id', storeIdFromUrl).not('supplier_id', 'is', null)
      ]);
      ...
      const balanceList = suppliers.map(s => {
        ...
        const totalCredit = sTrans.filter(t => t.is_credit === true).reduce((acc, t) => acc + Math.abs(Number(t.amount) || 0), 0)
        const totalPaid = sTrans.filter(t => t.type === 'debt_payment').reduce((acc, t) => acc + Math.abs(Number(t.amount) || 0), 0)
      ...
      console.error(err)
      if (!err.message.includes("invalid input syntax")) toast.error(`Σφάλμα: ${err.message}`)
  ...
  async function handleDeleteDebt(supplierId: string, supplierName: string) {
    ...
    try {
      const { error } = await supabase.from('transactions').delete()
        .eq('supplier_id', supplierId).eq('store_id', storeIdFromUrl)
      ...
      <div style={contentContainer}>
        ...
        <div style={headerRow}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h1 style={titleStyle}>Καρτέλες</h1>
              <p style={subtitleStyle}>ΥΠΟΛΟΙΠΑ ΠΡΟΜΗΘΕΥΤΩΝ</p>
            </div>
          </div>
              ...
              <div key={s.id} style={supplierCardStyle}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', marginTop: '15px' }}>
                    <button onClick={() => router.push(`/suppliers?store=${storeIdFromUrl}&edit=${s.id}`)} style={linkBtnStyle}>ΚΑΡΤΕΛΑ</button>
                    <button onClick={() => handleDeleteDebt(s.id, s.name)} style={linkBtnDangerStyle}>ΜΗΔΕΝΙΣΜΟΣ</button>
                  </div>
                </div>
                
                <div style={rightSideStyle}>
                  <p style={balanceStyle}>{s.balance.toFixed(2)}€</p>
                  <button 
...
// --- STYLES (PC & REDMI OPTIMIZED) ---
const iphoneWrapper: any = { 
  backgroundColor: colors.bgLight, 
  minHeight: '100%', // Διορθώνει το PC scroll
  width: '100%', 
  display: 'block',
  touchAction: 'pan-y' // Διορθώνει το Redmi scroll
};

const contentContainer: any = { maxWidth: '500px', margin: '0 auto', padding: '20px', paddingBottom: '120px' };
...
const subtitleStyle = { margin: 0, fontSize: '10px', color: colors.secondaryText, fontWeight: '700', letterSpacing: '1px' };
...
const totalLabelStyle = { margin: 0, fontSize: '11px', fontWeight: '700', color: '#fed7aa', letterSpacing: '1px' };
const totalAmountStyle = { margin: '8px 0 0 0', fontSize: '38px', fontWeight: '900', color: '#ffffff' };
const listHeaderStyle = { fontSize: '11px', fontWeight: '800', color: colors.secondaryText, textTransform: 'uppercase', letterSpacing: '0.5px', marginLeft: '5px' };
const supplierCardStyle: any = { backgroundColor: colors.white, padding: '18px', borderRadius: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'stretch', border: `1px solid ${colors.border}`, marginBottom: '10px' };
const supplierNameStyle = { fontWeight: '800', margin: 0, fontSize: '15px', color: colors.primaryDark };
const rightSideStyle: any = { textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'space-between' };
const balanceStyle = { fontWeight: '900', fontSize: '18px', color: colors.accentOrange, margin: 0 };
const payBtnStyle: any = { backgroundColor: '#eff6ff', color: colors.accentBlue, border: `1px solid #dbeafe`, padding: '8px 12px', borderRadius: '10px', fontSize: '11px', fontWeight: '900', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' };
const badgeStyle: any = { fontSize: '9px', fontWeight: '800', backgroundColor: '#f1f5f9', color: colors.secondaryText, padding: '4px 8px', borderRadius: '6px', marginTop: '6px', display: 'inline-block' };
...
const selectStyle: any = { 
  ...
  fontSize: '16px', // Zoom fix για Redmi
  fontWeight: '700', 
