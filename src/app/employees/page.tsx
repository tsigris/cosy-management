...
function EmployeesContent() {
  ...
  async function handleSave() {
    ...
    if (!formData.monthly_salary || Number(formData.monthly_salary) <= 0) {
      return alert('Ο μισθός είναι υποχρεωτικός!')
    }

    setLoading(true)
    
    // ΟΡΙΣΤΙΚΟ PAYLOAD: Μόνο στήλες που υπάρχουν στη βάση
    const payload = {
    ...
    try {
      ...
      alert("Κάτι πήγε στραβά: " + e.message)
    } finally {
      setLoading(false)
    }
  }
                       ...
                       <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                          {transactions.filter(t => t.employee_id === emp.id).map(t => (
                            <div key={t.id} style={historyItem}>
                               <span style={{ color: colors.secondaryText, fontWeight: '700' }}>
