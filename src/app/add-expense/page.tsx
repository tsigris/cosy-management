...
// 1. Το Component με όλη τη λειτουργία (Υπάλληλοι, Προμηθευτές κλπ)
function ExpenseFormFields() {
  ...
  if (fetching && employees.length === 0) return <div style={{ textAlign: 'center', padding: '40px' }}>Φόρτωση...</div>

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        <label style={labelStyle}>ΠΟΣΟ (€)</label>
        <input type="number" inputMode="decimal" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} style={inputStyle} />
      </div>
...
// 2. Η σελίδα που παρέχει το Suspense Boundary (για το Vercel)
export default function AddExpensePage() {
  ...
  return (
    <main style={{ padding: '20px', maxWidth: '500px', margin: '0 auto', fontFamily: 'sans-serif', backgroundColor: '#ffffff', minHeight: '100vh' }}>
      ...
      <Suspense fallback={<div style={{ textAlign: 'center', padding: '40px' }}>Φόρτωση...</div>}>
        <ExpenseFormFields />
      </Suspense>
    </main>
  )
