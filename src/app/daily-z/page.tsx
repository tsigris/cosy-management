...
export default function DailyZPage() {
  ...
  const [withdraw, setWithdraw] = useState('') // Ποσό για Τσέπη
  const [date, setDate] = useState(new Date().toISOString().split('T')[0])
  const [loading, setLoading] = useState(false)

  // Υπολογισμοί στην οθόνη
  const totalSales = Number(cashZ) + Number(posZ) + Number(noTax)

  async function handleSaveZ() {
    if (totalSales <= 0) return alert('Παρακαλώ συμπληρώστε τα έσοδα της ημέρας.')
    setLoading(true)

    // 1. Προετοιμασία Εσόδων (3 κινήσεις)
    ...
    // 2. Προετοιμασία Ανάληψης (Αποθήκευση ως 'pocket' για τον κουμπαρά)
    const pocketTransaction = Number(withdraw) > 0 ? [{
      amount: Number(withdraw),
      method: 'Μετρητά',
      notes: 'ΑΝΑΛΗΨΗ ΓΙΑ ΤΣΕΠΗ',
      type: 'expense',
      date,
      category: 'pocket' // Εδώ γίνεται η σύνδεση με τον κουμπαρά
    }] : []

    const { error } = await supabase.from('transactions').insert([...incomeTransactions, ...pocketTransaction])
    
    if (!error) {
      alert('Το ταμείο έκλεισε! Τα λεφτά προστέθηκαν στην τσέπη.')
    ...
    <main style={{ backgroundColor: '#f8fafc', minHeight: '100vh', padding: '16px', fontFamily: 'sans-serif' }}>
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '25px' }}>
          ...
          <h2 style={{ fontSize: '20px', fontWeight: '900', color: '#1e293b', margin: 0 }}>Κλείσιμο Ζ ημέρας</h2>
        </div>

        {/* ΕΙΣΠΡΑΞΕΙΣ */}
        <div style={sectionBox}>
          <p style={sectionTitle}>💰 ΕΣΟΔΑ (ΤΙ ΧΤΥΠΗΘΗΚΕ)</p>
          <div style={fieldBox}>
        ...
        <div style={{...sectionBox, backgroundColor: '#f0fdf4', borderColor: '#bbf7d0'}}>
          ...
          <div style={fieldBox}>
            <label style={{...labelStyle, color: '#166534'}}>ΠΟΣΟ ΠΟΥ ΠΑΙΡΝΩ ΜΑΖΙ ΜΟΥ</label>
        ...
        {/* ΗΜΕΡΟΜΗΝΙΑ */}
        <div style={fieldBox}>
          <label style={labelStyle}>ΗΜΕΡΟΜΗΝΙΑ Ζ</label>
          <input type="date" value={date} onChange={e => setDate(e.target.value)} style={{...inputStyle, fontSize: '16px'}} />
        </div>

        {/* TOTAL */}
        <div style={totalBox}>
          <p style={labelStyle}>ΣΥΝΟΛΙΚΟΣ ΤΖΙΡΟΣ</p>
          <h2 style={{ fontSize: '32px', margin: 0, fontWeight: '900', color: '#1e293b' }}>{totalSales.toFixed(2)}€</h2>
        </div>

        <button onClick={handleSaveZ} disabled={loading} style={saveBtn}>
          {loading ? 'Αποθήκευση...' : 'ΟΡΙΣΤΙΚΟΠΟΙΗΣΗ Ζ'}
        </button>
      </div>
    </main>
  )
...
const sectionTitle = { fontSize: '10px', fontWeight: '900', color: '#64748b', marginBottom: '15px', letterSpacing: '1px' };
...
const totalBox = { textAlign: 'center' as const, padding: '20px', marginBottom: '20px', backgroundColor: '#f8fafc', borderRadius: '20px' };
