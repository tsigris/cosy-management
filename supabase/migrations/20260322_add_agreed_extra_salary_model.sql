...
alter table if exists public.fixed_assets
  add column if not exists agreed_extra_salary numeric not null default 0;

update public.fixed_assets
set agreed_extra_salary = 0
where agreed_extra_salary is null;
...
as $function$
  ...
  employee_rows as (
    select
      ...
      s.name,
      s.monthly_salary,
      s.agreed_extra_salary,
      25::numeric as insurance_pct,
      coalesce((s.monthly_salary * 0.25), 0)::numeric as insurance_amount,
      coalesce((s.monthly_salary + (s.monthly_salary * 0.25) + s.agreed_extra_salary), 0)::numeric as total_monthly_cost,
      coalesce(((s.monthly_salary + (s.monthly_salary * 0.25) + s.agreed_extra_salary) / 30.0), 0)::numeric as daily_cost,
      coalesce((((s.monthly_salary + (s.monthly_salary * 0.25) + s.agreed_extra_salary) / 30.0) * pm.days_in_period), 0)::numeric as period_cost,
      case
        when tr.period_turnover > 0 then (((((s.monthly_salary + (s.monthly_salary * 0.25) + s.agreed_extra_salary) / 30.0) * pm.days_in_period) / tr.period_turnover) * 100)::numeric
        else 0::numeric
      ...
      coalesce(
        jsonb_agg(
          jsonb_build_object(
            ...
            'agreed_extra_salary', er.agreed_extra_salary,
